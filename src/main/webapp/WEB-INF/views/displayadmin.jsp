            ...
            <c:forEach items="${userList}" var="user" varStatus="loop">
                <tr>
                    ...
                    <td class="py-2 px-4 border-b border-gray-600">
                        <c:if test="${user.bindphoto != null}">
                            <img src="data:image/jpeg;base64,${user.bindphoto}" alt="User Photo" class="w-8 h-8 rounded-full" />
                        </c:if>
                        <c:if test="${user.bindphoto == null}">
                            <p>No photo available.</p>
                        </c:if>
                    </td>
                    <td class="py-2 px-2 border-b border-gray-600">${user.name}</td>
                    <td class="py-2 px-4 border-b border-gray-600">${user.email}</td>
                    <td class="py-2 px-4 border-b border-gray-600">
                    <c:choose>
                    <c:when test="${user.email == 'yupaingmoe2003.npt@gmail.com' }">
                    	&lt;SuperAdmin&gt;
                    </c:when>
                    <c:otherwise>
                    
                        <a href="/StudentManagement_SpringJPA/deleteuser/${user.id}" class="text-red-300 hover:underline">Remove</a>
